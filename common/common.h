...
struct common_params {
    ...
    bool sparse_attn       = false; // use the sparse attention indexer where the model provides one
...
struct common_prompt_checkpoint {
    ...
    // slot checkpoint store: the blobs live in a record file, the data_* vectors stay empty
    bool     on_disk  = false;
    uint64_t disk_id  = 0;
    uint64_t off_tgt  = 0;
    uint64_t size_tgt = 0;
