...
extern "C" {
    ...
    // MoE expert-routing observation callback: invoked by the CPU mul_mat_id with
    // the ids tensor of a host-resident MoE layer's gate mul_mat_id, right before
    // that op computes. Used by the llama MoE GPU expert cache to drive slot
    // placement from the actual router decisions.
